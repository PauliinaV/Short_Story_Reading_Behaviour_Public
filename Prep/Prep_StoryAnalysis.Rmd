...
AllStories <- data.frame(
    ...
    SDWordFrequency = NA,
    NumberOfWords = NA,
    NumberOfSentences = NA,
    AverageSentenceLength = NA,
    SDSentenceLength = NA,
...
In addition to word frequency, we want estimates on the number of different sentences in the short stories and the average sentence length.
...
for (story in 1:length(AuthorList)) {
    ...
    AllStories[AllStories$Author == AuthorList[story], "NumberOfSentences"] <- length(Sentences)
    # count sentence length
    sentences_data <- data.frame(
        Sentences = Sentences,
        NumberOfWords = NA
    )
    sentences_data$NumberOfWords <- str_count(sentences_data$Sentences, " ")
    # counting spaces misestimates sentence length by 1
    ## because there is no space at the end of the sentence
    ### Therefore, we +1 to each sentence word count
    sentences_data$NumberOfWords <- sentences_data$NumberOfWords + 1
    # count average sentence length:
    AllStories[AllStories$Author == AuthorList[story], "AverageSentenceLength"] <- mean(
        sentences_data$NumberOfWords
    )
    AllStories[AllStories$Author == AuthorList[story], "SDSentenceLength"] <- sd(
        sentences_data$NumberOfWords
    )
}
```

Sentence information for the ninth story were calculated separately. We add this information to the AllStories dataframe:

```{r}
AllStories[AllStories$Author == "Unnamed", "NumberOfSentences"] <- 460
AllStories[AllStories$Author == "Unnamed", "AverageSentenceLength"] <- 16.2283
AllStories[AllStories$Author == "Unnamed", "SDSentenceLength"] <- 11.7276
```

The average number of sentences in a story is `r round(mean(AllStories$NumberOfSentences), 2)` and the average number of words in a sentence is `r round(mean(AllStories$AverageSentenceLength), 2)`

```{r}
ggplot(AllStories, aes(x = NumberOfSentences, y = AverageSentenceLength, colour = Author)) +
    geom_point() +
    theme_classic()
cor.test(AllStories$AverageSentenceLength, AllStories$NumberOfSentences)
```

Average sentence length and number of sentences are significant negatively correlated. This means that stories that had longer sentences were likely to have fewer sentences than stories with shorter sentences.
