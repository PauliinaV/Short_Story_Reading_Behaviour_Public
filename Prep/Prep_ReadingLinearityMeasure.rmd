...
To measure frequency of nonlinear navigation, we calculate how often a nonnavigation event, a linear navigation event, or a different type of nonlinear navigation event is followed by nonlinear navigation. Simply put, previous event's linearity does not equal current event's linearity, and the current event is nonlinear. This measure therefore reflects the frequency of initiating nonlinear navigation during reading of the story.
...
grouped_tracking_data$IsReadingSetUpChange <-
    (
        grouped_tracking_data$NBFirstTotalPagesInSection !=
            (lag(grouped_tracking_data$NBFirstTotalPagesInSection, 1))
    )
...
ggplot(tracking_data, aes(x = Linearity, group = UserId, fill = UserId)) +
    geom_bar(position = position_dodge(), stat = "count") +
    theme_classic()
```
...
ggplot(tracking_data, aes(x = StartsNonlinearity, group = UserId, fill = UserId)) +
    geom_bar(position = position_dodge(), stat = "count") +
    theme_classic()
```
...
```{r}
plot1 <- ggplot(participant_navigation_counts, aes(x = IsNonlinearNavigationCount, y = StartsNonlinearityCount, colour = UserId)) +
    geom_point() +
    theme_classic()
ggplotly(plot1)
```
...
linearity_measure_data <- linearity_measure_data %>%
    group_by(UserId, StoryId, ReadingSessionNumber, ReadingBlockNumber) %>%
    summarise(
