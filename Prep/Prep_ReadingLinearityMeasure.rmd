...
To measure frequency of nonlinear navigation, we calculate how often a nonnavigation event or a linear navigation event is followed by nonlinear navigation. This measure therefore reflects the frequency of initiating nonlinear navigation during reading of the story.
...
grouped_navigation_blocks_data[1, "IsNewUser"] <- TRUE
grouped_navigation_blocks_data[1, "IsNewSession"] <- TRUE
grouped_navigation_blocks_data[1, "IsReadingSetUpChange"] <- TRUE
```
...
LastRow <- nrow(grouped_navigation_blocks_data)
grouped_navigation_blocks_data[LastRow, "PagesTurned"] <- (
    grouped_navigation_blocks_data[LastRow, "LastPage"] -
        grouped_navigation_blocks_data[LastRow, "FirstPage"]
)
# Check PagesTurned
...
- Linearity tells us what is the type of linearity (regression, forward leap, or linear/nonnavigation)
- StartsNonlinearity tells us whether the event initiates nonlinear navigation following linear navigation, nonnavigation, or nonlinear navigation of a different type.
...
for (row in 1:nrow(wrangled_tracking_data)) {
    if(row == 1 |
        wrangled_tracking_data[row, "IsNewUser"] |
            wrangled_tracking_data[row, "IsNewSession"] |
                wrangled_tracking_data[row, "IsReadingSetUpChange"]) {
        # first row, new user, new session, or new set up
        if(wrangled_tracking_data[row, "IsNonlinearNavigation"]) {
            # nonlinear navigation
            wrangled_tracking_data[row, "StartsNonlinearity"] <-
                TRUE
        } else {
            # not nonlinear
            wrangled_tracking_data[row, "StartsNonlinearity"] <-
        ...
        if ((wrangled_tracking_data[row - 1, "Linearity"] !=
                wrangled_tracking_data[row, "Linearity"]) &
                wrangled_tracking_data[row, "IsNonlinearNavigation"]) {
...
Finally, to create a measure for linearity to use in the analysis, we filter the df to only include one event per page-view.
...
* Select one event per page-view that includes information on linearity (in particular, "StartsNonlinearity")
...
## 8143 rows (full data 9846 rows)
linearity_measure_data <- wrangled_tracking_data %>%
    filter(Engagement == "Engagement")
# Select one event per each page-view:
## group events by
### UserId, ReadingSessionNumber, and ReadingBlockNumber (page-view indicator)
#### and summarise important variables into columns
linearity_measure_data <- linearity_measure_data %>%
    group_by(UserId, ReadingSessionNumber, ReadingBlockNumber) %>%
    summarise(
        ...
        FirstCumReadingSessionTime = first(CumReadingSessionTime),
...
write.csv2(linearity_measure_data,
