...
IMI_item_correlations <- cor(
    IMI_data[, c(2:length(IMI_data))]
)
# not really polycor but fix this:
IMI_item_polychor_correlations <- IMI_item_correlations
...
```{r}
corrplot(round(IMI_item_polychor_correlations, 2), method = "circle", type = "lower")
```
...
# Effort
IMI_effort_subscale <- dplyr::select(
    IMI_data,
    questionnaire22_13,
    questionnaire22_17,
    questionnaire22_24,
...
# Pressure
IMI_pressure_subscale <- dplyr::select(
    IMI_data,
    questionnaire22_12,
    questionnaire22_25
)
...
- The subscale of 'effort' has 'good' internal consistency (Cronbach's Alpha = `r round(IMIEffortCronbach$alpha, 3)`, Confidence interval = `r round(IMIEffortCronbach$ci, 2)`)
- The subscale of 'pressure' has 'unacceptable' internal consistency (Cronbach's Alpha = `r round(IMIPressureCronbach$alpha, 3)`, Confidence interval = `r round(IMIPressureCronbach$ci, 2)`)
...
'Interest', 'competence', 'effort', and 'autonomy' show sufficient internal consistency. However, the subcomponent of 'pressure' has a very low internal consistency score. This is likely to be due to the fact that the IMI only includes two items measuring participants' feelings of pressure and tension. Furthermore, these two items are not significantly correlated with each other: `r cor.test(IMI_pressure_subscale$questionnaire22_12, IMI_pressure_subscale$questionnaire22_25)`. Due to the low internal consistency of 'pressure', we do not calculate a score for it and the 'pressure' component is not used in analyses.

### IMI scores

Next, we calculate each participants' score on each of the IMI subscales by summing item scores together.

We create each participant a score on each of the subscales by summing item scores together. The IMI scores are named as 'SInterest', 'SCompetence', 'SEffort', and 'SAutonomy' (S: situational motivation) to distinguish them from the IMI-R subscale scores.
...
IMI_scores <- IMI_data %>%
    mutate(
        ...
        SEffort = rowSums(.[names(IMI_effort_subscale)]),
        SAutonomy = rowSums(.[names(IMI_autonomy_subscale)])
    ) %>%
    dplyr::select(UserId, SInterest, SCompetence, SEffort, SAutonomy)
```

Inspect participants' IMI scores:

```{r 'Interest IMI scores'}
# interest subscale
...
# visualise
ggplot(IMI_scores, aes(x = UserId, y = SInterest)) +
    geom_point() +
    theme_classic()
```
...
# visualise
ggplot(IMI_scores, aes(x = UserId, y = SCompetence)) +
    geom_point() +
    theme_classic()
```
...
mean(IMI_scores$SEffort)
sd(IMI_scores$SEffort)
range(IMI_scores$SEffort)
# visualise
ggplot(IMI_scores, aes(x = UserId, y = SEffort)) +
    geom_point() +
    theme_classic()
```

The average situational effort score is `r round(mean(IMI_scores$SEffort), 2)` (*SD* = `r round(sd(IMI_scores$SEffort), 2)`, *range* = `r round(range(IMI_scores$SEffort), 2)`). The participants show a lot of variation in their scores for the effort subscale. The average situational effort score divided by the amount of items is `r round((mean(IMI_scores$SEffort)/5), 2)`, which corresponds with the neutral answer option on the scale (scale: 1 (*not at all true*) to 7 (*very true*)).
...
# visualise
ggplot(IMI_scores, aes(x = UserId, y = SAutonomy)) +
    geom_point() +
    theme_classic()
```
...
```{r 'IMI correlations between scores'}
...
ggplot(IMI_scores, aes(x = SInterest, y = SEffort)) +
    geom_point() +
    geom_smooth() +
    theme_classic() +
    ggtitle("IMI: situational interest and effort scores")
ggplot(IMI_scores, aes(x = SCompetence, y = SEffort)) +
    geom_point() +
    geom_smooth() +
    theme_classic() +
    ggtitle("IMI: situational competence and effort scores")
ggplot(IMI_scores, aes(x = SAutonomy, y = SEffort)) +
    geom_point() +
    geom_smooth() +
    theme_classic() +
    ggtitle("IMI: situational autonomy and effort scores")
```
...
In contrast, the 'effort' subscale is not correlated with any of the other components. Similarly to the IMI-R, participants may have found the 'effort' items ambiguous to respond to as putting a lot of effort in reading a text can indicate of positive investment in the activity, or a negative burden of completing the short story. Due to this ambiguity, the 'effort' score is not used in analyses.

```{r 'IMI remove effort component'}
IMI_scores <- dplyr::select(
    IMI_scores,
    -SEffort
)
```
...
```{r 'code and inspect access to digital devices'}
...
ggplot(eexp_data, aes(x = UserId, y = Eexp1LongformEFreq)) +
    geom_point() +
    theme_classic()
```
...
ggplot(eexp_data, aes(x = UserId, y = Eexp2DeviceRecFreq)) +
    geom_point() +
    theme_classic()
```
...
```{r 'correlation between eexp measures'}
cor(eexp_data[, c("Eexp1LongformEFreq", "Eexp2DeviceRecFreq")])
plot(eexp_data$Eexp1LongformEFreq, eexp_data$Eexp2DeviceRecFreq)
```
...
```{r 'inspect access to digital devices and eexp measures'}
