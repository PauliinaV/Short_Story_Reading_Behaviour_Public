...
* Consists of the following subcomponents:
    * 'Interest': participants' interest in the short story that they read, e.g. *I thought that reading this story was quite enjoyable*
    * 'Competence': Perceived competence in reading and difficulty of the read short story, e.g. *I was pretty skilled at reading the story*
    * 'Autonomy': Perceived autonomy in text selection and the ways in which the short story was read, e.g. *I read the story because I had to*
...
```{r 'external-file-call'}
if (exists("AnalysisFilePath")) {
    mypath_SSRBP <- dirname(dirname(AnalysisFilePath))
} else if (grepl("Prep", getwd())) {
    # working directory is this exact file location
    mypath_SSRBP <- dirname(getwd())
} else if (grepl("Short_Story_Reading_Behaviour_Public", getwd())) {
    # working directory is the main folder
    mypath_SSRBP <- getwd()
} else {
    # the working directory needs to be configured
    # change it here so that mypath_SSRBP indicates main folder location
    # getwd()
    # mypath_SSRBP <- XXX
}
```
...
```{r 'change-variable types', echo=FALSE}
...
```{r 'correlation-plot-IMI-R', eval=FALSE}
...
Next, we check the internal consistency of the IMIR overall scale and each of the subscales (interest and competence) by computing Cronbach's Alpha.
...
The full scale Cronbach's Alpha is = `r round(IMIRFullScaleCronbach$alpha, 3)` with a confidence interval of `r round(IMIRFullScaleCronbach$ci, 2)`. Similarly to IMI, IMI-R fullscale has 'good' internal consistency.
...
```{r 'IMI-R-score-correlations', eval=FALSE}
...
Next, we check the internal consistency of the IMI overall scale and each of the subscales (interest, competence, and autonomy) by computing Cronbach's Alpha.
...
The full scale Cronbach's Alpha is = `r round(IMIFullScaleCronbach$alpha, 3)` with a confidence interval of `r round(IMIFullScaleCronbach$ci, 2)`. The fullscale therefore has 'excellent' internal consistency.
...
```{r 'Interest-IMI-scores', eval=FALSE}
# interest subscale
...
```{r 'IMI-competence-score', eval=FALSE}
# competence subscale
## max score on the scale: 6*7 = 42, min score 6*1 = 6
...
```{r 'IMI-correlations-between-scores', eval=FALSE}
...
for (k in nameslist) {
    eexp_data[, k] <- dplyr::recode(eexp_data[, k],
        "Every day" = 5,
...
```{r 'visualise-eexp-measure-1', eval=FALSE}
...
```{r 'visualise-eexp-measure-2', eval=FALSE}
...
```{r 'correlation-between-eexp-measures', eval=FALSE}
...
```{r 'inspect-access-to-digital-devices-and-eexp-measures', eval=FALSE}
