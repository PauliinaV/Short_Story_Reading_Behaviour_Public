...
We then calculate absolute reading rate (baseline speed deducted from reading speed):

```{r}
speed_measure_data$ReadingRate <- (
    speed_measure_data$EngagedReadingSpeed
    - speed_measure_data$AdjustedBaselineSpeed)
speed_measure_data$AbsReadingRate <- abs(
    speed_measure_data$ReadingRate
    )
```
...
Variance in absolute reading rate

```{r}
ggplot(reading_speed_measure_data, aes(x = StartLocation, y = AbsReadingRate)) +
    geom_line() +
    facet_wrap(~UserId)
```

By-participant variance in absolute reading rate.
