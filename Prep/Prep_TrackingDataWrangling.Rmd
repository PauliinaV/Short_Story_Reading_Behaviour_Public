...
**Engagements.** Engagements that have a duration of >5min indicate of a missing tracking event. A reading session ending in an 'engagement' indicates that the final disengagement event (triggered by the closure of the e-reader system, for example, event of type 'logout') is missing. Similarly, a disengagement event would have followed the engagement if the participant had become inactive: the e-reader system automatically masks the text after 5 minutes of inactivity which is recorded as a disengagement event of type 'blur'.

Missing events may occur when the e-reader system is abruptly closed, or when network connection fails*, and the system does not have the time to record an event. Indeed, correctly recorded engagements should last for a maximum of 5 minutes before the e-reader system masks the text. Therefore, excessive engagement durations are checked by filtering the dataset for engagements that last for longer than 5.1 minutes.
...
length(excessive_engagement_durations_subdata) # 7 excessive engagement durations
```
