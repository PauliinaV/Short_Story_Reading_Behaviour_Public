...
reading_session_details <- data.frame(
    ...
    SessionStartDate = numeric(),
    SessionStartTime = numeric(),
    SessionEndDate = numeric(),
    ...
    for (session in unique(subset_user$ReadingSessionNumber)) {
        ...
        temporary_df <- subset_readingsession %>%
            summarise(
                ...
                SessionStartDate =
                    as.numeric(first(subset_readingsession$Date)),
                SessionStartTime = 
                    as.numeric(first(subset_readingsession$Time)),
                SessionEndDate =
                    as.numeric(last(subset_readingsession$Date)),
                SessionEndTime =
                    as.numeric(last(subset_readingsession$Time))
            ) %>%
            mutate(
...
```{r, 'fix-date-time-variable-types'}
reading_session_details$SessionStartDate2 <-
    as.Date(
       reading_session_details$SessionStartDate,
        origin = "1970-01-01"
    )
reading_session_details$SessionEndDate2 <-
    as.Date(
       reading_session_details$SessionEndDate,
        origin = "1970-01-01"
    )

reading_session_details$TimeBetweenReadingSessions <- NA
 reading_session_details <-
    reading_session_details[
        with(
             reading_session_details,
            order(UserId, SessionStartDate, ReadingSessionNumber)
        ),
]
for (row in 1:nrow(reading_session_details)) {
    if (row > 1) {
        if ( reading_session_details[row, "UserId"] == reading_session_details[row - 1, "UserId"]) {
            reading_session_details[row, "TimeBetweenReadingSessions"] <- (
                                reading_session_details[row, "SessionStartDate2"] -
                                    reading_session_details[row-1, "SessionEndDate2"]
                            )
        }
    }
}
...
op <-
    options(digits.secs = 3)
raw_tracking_data$SessionStartTime2 <-
    strptime(
        reading_session_details$SessionStartTime
    )
```
...
## Check the amount of visible words on a page

The text masking threshold of 5 minutes is based on the assumption that participants are unlikely to use the e-reader system on such large devices that the visible word count on a page exceeds 1000 words, and thus, nearly all pages should be readable by an average reader at 200wpm. We check if this assumption holds by plotting the amount of visible words on a page.

```{r}
VisibleColumnLabels <- c(OneColumn = "One column", TwoColumns = "Two columns")
(VisibleWordsPlot <- ggplot(artefacts_removed_tracking_data, aes(x = VisibleWordCount, fill = VisibleColumns)) +
    geom_histogram(bins = 100) +
    coord_cartesian(xlim = c(0, 1100), expand = FALSE) +
    geom_vline(xintercept = 1000) +
    labs(x = "Visible words on a page-view", y = "Count", fill = "Visible columns of text") +
    scale_colour_discrete(labels = VisibleColumnLabels) +
    figure_theme_without_legend_position +
    theme(
        legend.position = c(0.65, 0.8)
    ))
# save the figure
#ggsave("Fig_VisibleWords.png", plot = VisibleWordsPlot, dpi = 300, height = 10, width = 10, units = "cm")
any(artefacts_removed_tracking_data$VisibleWordCount > 1000)
```

The assumption holds considering that none of the observations exceed 1000 visible words on a page.
