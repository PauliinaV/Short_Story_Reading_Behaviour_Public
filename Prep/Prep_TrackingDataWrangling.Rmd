...
## fix numeric columns that were automatically categorised as chr
raw_tracking_data[, c(
    "BaselineSpeed",
...
**Engagements.** Engagements that have a duration of >5min indicate of a missing tracking event.
...
* An engagement event that has a duration of >5min in the middle of a reading session is similarly indicative of a missing event: the e-reader system automatically masks the text after 5 minutes of inactivity (event type 'blur'), therefore, if an engagement event lasts for longer than 5 minutes, the text mask event is missing from the tracking data.

Missing events occur when the system doesn't have the time to record an event, for example, when the e-reader system is abruptly closed or when network connection fails (see Note below). Indeed, correctly recorded engagements should last for a maximum of 5 minutes before the e-reader system masks the text. Therefore, excessive engagement durations are checked by filtering the dataset for engagements that last for longer than 5.1 minutes.
...
1. 'AdjustedEventDuration' shows the suggested new event duration
    ...
    * If an event is not the last in a reading session or an excessive engagement duration, AdjustedEventDuration = DurationMinutes
...
Next, we will check that AdjustedEventDuration does what we expect it to do. 
...
2. 'AdjustedNavigationBlockDuration' shows the suggested adjustment to NavigationBlockDuration
    * This variable is calculated by summing AdjustedEventDuration for each UserId and each NavigationBlockNumber
...
Finally, create a new dataframe without the unadjusted event duration and navigation block duration to avoid confusion.
...
Events were given engagement labels in the data pre-processing stage (see file __ in the Info folder for detail), however, some rare events were mistakenly classified as disengagements. During the opening of the e-reader system, an event was occasionally categorised as a 'disengagement' even though the text was visible to the participants. These events are first identified by inspection of 'unusual disengagements' which
1. don't have a clear disengagement-linked event type (such as 'blur' or 'close', see more information on events in ___ in the Info folder),
2. occur when the page is open (IsPageOpen == TRUE), and
3. occur when disengagement linked tests are FALSE (e.g. the page is not blurred: !IsBlurred)

## CHECK: 
```{r, inaccurately categorised disengagements}
adjusted_durations_data$IsUnusualDisengagement <-
    (
        (adjusted_durations_data$Engagement == "Disengagement") &
            (adjusted_durations_data$Type != "blur" &
                adjusted_durations_data$Type != "openMenu" &
                adjusted_durations_data$Type != "inactiveTimeout" &
                adjusted_durations_data$Type != "close" &
                adjusted_durations_data$Type != "logout" &
                adjusted_durations_data$Type != "openBook" &
                adjusted_durations_data$Type != "bookDialogOpen" &
                adjusted_durations_data$Type != "openInformation") &
            (adjusted_durations_data$IsPageOpen &
                !adjusted_durations_data$IsBlurred &
                !adjusted_durations_data$IsDialogOpen &
                !adjusted_durations_data$IsMenuOpen &
                !adjusted_durations_data$IsInactive)
    )
```
raw_tracking_data$Engagement <-
