...
Calculate some additional variables that will be useful later.
...
raw_tracking_data$EndLocation <- (
    raw_tracking_data$StartLocation +
        raw_tracking_data$VisibleCharacterCount
)
raw_tracking_data$VisibleColumns <- ifelse(
    raw_tracking_data$WindowWidth > 1192,
    "TwoColumns",
    "OneColumn"
)
```
...
# book_information_data <- read.csv()
# the above should include storyId, chlength, and wordlength
# raw_tracking_data <- merge(
# raw_tracking_data,
# book_information_data,
# by="StoryId"
# )
# raw_tracking_data$Percentage <- (
# raw_tracking_data$StartLocation/raw_tracking_data$ChLength
# )
```
...
4. The event occurs when disengagement-linked tests are FALSE (e.g. the text is not blurred: !IsBlurred), and
...
These events are part of the opening sequence of a new reading session (IsOpeningEvent). The e-reader system was expecting an 'openPage' event to signify the opening of a new page, but instead the page had been loaded previously. This confusion caused the events to be categorised as disengagements rather than engagements. To fix this mistake, the 10 unusual disengagements are turned into engagements:
...
## Create tests for later usage

Tests to check if an event is from a new user, reading session, or reading set up are useful across the analysis.

```{r, create tests}
tracking_data_artefacts_removed$IsNewUser <-
    (
        tracking_data_artefacts_removed$UserId !=
            (lag(tracking_data_artefacts_removed$UserId, 1))
    )
tracking_data_artefacts_removed$IsNewSession <-
    (
        tracking_data_artefacts_removed$ReadingSessionNumber !=
            (lag(tracking_data_artefacts_removed$ReadingSessionNumber, 1))
    )
tracking_data_artefacts_removed$IsReadingSetUpChange <-
    (
        tracking_data_artefacts_removed$TotalPagesInSection !=
            (lag(tracking_data_artefacts_removed$TotalPagesInSection, 1))
    )
# first event is TRUE for all 3 tests
tracking_data_artefacts_removed[1, "IsNewUser"] <- TRUE
tracking_data_artefacts_removed[1, "IsNewSession"] <- TRUE
tracking_data_artefacts_removed[1, "IsReadingSetUpChange"] <- TRUE
```
