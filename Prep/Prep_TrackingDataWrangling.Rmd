...
All events that have been tracked outside the e-reader (IsReading == FALSE) need to be removed. The e-reader tracks some usage of the website landing page in order to determine when participants leave the e-reader. However, these events are not useful for the analysis and so they are removed:
...
Removing the !IsReading events reduces the amount of events from 10,390 to 9,244
...
**Disengagements.** If the last event in a reading session is a disengagement, such as 'close' or 'logout' (see event types in info folder, document X), the duration of the event is recorded as 0. This is because a disengagement at the end of a reading session indicates of the participants' intention to leave the e-reader system, and so its duration is arbitrary.
...
The above code shows that `r sum(raw_tracking_data$IsAdjustedDuration)` event durations were adjusted.
...
Finally, create a new dataframe without unadjusted event duration and navigation block duration to avoid confusion. 'AdjustedEventDuration' and 'AdjustedNavigationBlockDuration' are then renamed to 'DurationMinutes' and 'NavigationBlockDuration' for simplicity.
...
(1) Disengagement: the participant is not using the e-reader. The text is not visible, or the user is outside of the e-reader.
(2) Dialog: the participant is using the menus or responding to the location prompt.
(3) Engagement: the participant is engaged in using the e-reader, the text is visible and the e-reader is active.
(4) DisruptedEngagement: the participant is engaged in using the e-reader, but only for 15 seconds or less continuously. The participant may have opened the e-reader by accident, or they may not be ready to resume the reading activity. The threshold of 15s is used in accordance to a study by @Iqbal2007 who used >15 seconds as an indicator of resumed engagement in the primary activity during a work engagement study.

The raw data includes a variable called 'Engagement' which tells us about the engagement category of each event. However, DisruptedEngagements are not differentiated from Engagements in the raw data. Therefore, we adjust 'Engagement' to include the DisruptedEngagement category.

In order to do this, we need to determine how long each engagement category lasts continuously. Participants may have multiple events of the same engagement type consecutively and, for example, to check whether an engagement is longer than 15 seconds, these events need to be grouped together and given a continuous duration. We call this 'continuous engagement duration' (or occassionally, CED for short).
...
Check if the loop had an unforeseen cases:
...
We can then calculate continuous durations into a new dataframe, and merge it with adjusted_durations_data:
...
We then remove the original engagement category value from the data to reduce mistakes. For simplicity, we then rename the AdjustedEngagement variable as 'Engagement':
...
Some of the participants' reading sessions are very short, and therefore they are identified as potential artefacts. This may be due to participants having the e-reader open on a separate tab on their computer or mobile device, if the tab is momentarily opened and then closed, the e-reader records a new event in a new reading session

```{r}
hist(reading_session_details$EngagementProportion, breaks = 100)
```

These potential reading session artefacts are identified based on criteria:
...
We save information about reading sessions for usage in the manuscript:
...
Save dataset of reading session artefacts. The df has already been saved and so the below code is not run:
...
Remove reading session artefacts:
...
```{r 'add column and end location information, cumulative time'}
...
ggplot(participant_engagement_time) +
    ...
    ggtitle("Total engagement time in hours")
```
