...
reading_frequency_data <- filter(reading_frequency_data, TimeBetweenReadingSessions != 0)
```

sample size: *n* = `r length(unique(reading_frequency_data$UserId))`
...
First, we build the full structure. Refer to the beginning of this script to see information on the full structure. The full model includes 13 parameters, and `r nrow(reading_frequency_data)` observations.

TOO LITTLE DATA ON READING FREQUENCY -- 37/13 < 20 observations per parameter. Do not analyse reading frequency

```{r}
FullStructure <- (
    "TimeBetweenReadingSessions ~ DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc + (1 | StoryId) + (1 | UserId)"
)
```

```{r}
ReadingFrequency_FullModel <- lmer(
    FullStructure,
