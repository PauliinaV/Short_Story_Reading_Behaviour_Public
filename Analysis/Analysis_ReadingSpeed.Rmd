...
We then assess whether the nonnormality of residuals could be addressed by using a gamma distributed GLMM rather than lmer. To do this, we add +1 to the outcome variable to make sure that all observations are positive. Similarly to the square-root transformation, we begin with a full model structure and then select the predictors backward stepwise until convergence:

```{r}
FullStructure_RC_Gamma <- (
    "RelativeSpeedDifference + 1 ~ DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Condition : SCompetence.sc + CInterest.sc : SCompetence.sc + Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc : SCompetence.sc + (1 | StoryId) + (1 | UserId) + (1 | WindowWidth)"
)
```
...
The model is singular, and so we select it backwards stepwise. The selection is done off-script for brevity.

```{r}
SimplifiedStructure_RC_Gamma <- ("RelativeSpeedDifference + 1 ~ 1 + (1 | StoryId) + (1 | UserId) + (1 | WindowWidth)")
```
...
The Gamma model does not reach convergence, even when all predictors have been removed and the model only has the intercept and random effects. This is because the random intercept of 'WindowWidth' is accounting for zero variance in the model, resulting in singularity. This is a common occurrence [@Bolker2022]. In this case, the random effect can be added in the model as a fixed effect to assess if this rectified the singularity issue.

We attempt to fit Gamma model again from the full model by using WindowWidth as a continuous fixed effect, rather than as a categorical random effect. This process is not done for the full model in lmer because the removal of the random intercept of WindowWidth is unlikely to affect the nonnormality of residuals.
...
FullStructure_RC_Gamma_Adjusted <- (
    "RelativeSpeedDifference + 5 ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Condition : SCompetence.sc + CInterest.sc : SCompetence.sc + Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc : SCompetence.sc + (1 | StoryId) + (1 | UserId)"
)
```

```{r}
Speed_RC_SelectedModel_Gamma_Adjusted <- glmer(
    FullStructure_RC_Gamma_Adjusted,
    data = reading_speed_measure_data,
    family = Gamma(),
    control = glmerControl(optimizer = "bobyqa")
...
```{r}
summary(Speed_RC_SelectedModel_Gamma_Adjusted)
```

The full model with gamma distribution and WindowWidth as a fixed rather than a random effect reaches convergence and is no longer singular. Normality of residuals is not an assumption for GLMMs so we do not test for it. Instead, we test for Over- and under dispersion:
...
simulated_model_output <- simulateResiduals(fittedModel = Speed_RC_SelectedModel_Gamma_Adjusted, plot = F)
```
