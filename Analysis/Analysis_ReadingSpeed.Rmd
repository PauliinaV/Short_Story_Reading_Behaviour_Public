...
First, we build the full structure. Refer to the beginning of this script to see information on the full structure. The full model includes 16 parameters (including two random effects and categorical variable levels), and `r nrow(reading_speed_measure_data)` observations. Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r nrow(reading_speed_measure_data)`/16 = `r round(nrow(reading_speed_measure_data)/16, 2)`
...
```{r 'run-fullstructure-model'}
Speed_RC_FullModel_nested <- lmer(
    FullStructure_RC_nested,
...
```{r, include=FALSE}
summary(Speed_RC_FullModel_nested)
fixef(Speed_RC_FullModel_nested)
# plot(Speed_RC_FullModel_nested)
# plot(reading_speed_measure_data$ReadingRate, fitted(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
check_collinearity(Speed_RC_FullModel_nested)
plot(check_collinearity(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
# cooksd <-
#     cooks.distance(Speed_RC_FullModel_nested)
# plot(cooksd,
#     pch = "*",
#     cex = 2,
...
```{r, include=FALSE}
check_outliers(Speed_RC_FullModel_nested)
plot(check_outliers(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
check_heteroscedasticity(Speed_RC_FullModel_nested)
plot(check_heteroscedasticity(Speed_RC_FullModel_nested))
```
...
```{r}
leveneTest(residuals(Speed_RC_FullModel_nested) ~ reading_speed_measure_data$Condition)
```

```{r}
leveneTest(residuals(Speed_RC_FullModel_nested) ~ reading_speed_measure_data$NativeEnglish)
```
...
```{r, include=FALSE}
random_intercept_UserId <- ranef(Speed_RC_FullModel_nested)$UserId$`(Intercept)`
...
```{r, include=FALSE}
random_intercept_StoryId <- ranef(Speed_RC_FullModel_nested)$StoryId$`(Intercept)`
...
```{r, include=FALSE}
# plot(resid(Speed_RC_FullModel_nested), fitted(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
# qqnorm(residuals(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
# hist(resid(Speed_RC_FullModel_nested), breaks = 100)
```
...
skewness(resid(Speed_RC_FullModel_nested))
# 3 - normal, <3 playkurtic, >3 leptokurtic
kurtosis(resid(Speed_RC_FullModel_nested))
```
...
```{r, include=FALSE}
# Shapiro Wilk Test
check_normality(Speed_RC_FullModel_nested)
```
...
speed_effectplot_electronicexperiencescomp <-
    interact_plot(Speed_RC_FullModel_nested,
        pred = SCompetence.sc,
...
First, we build the additive model structure. Refer to the beginning of the event properties section of this script to see information on how the model is built.

The model includes 10 parameters before adding interactions, and `r nrow(reading_speed_measure_data)` observations. Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r nrow(reading_speed_measure_data)`/10 = `r round(nrow(reading_speed_measure_data)/10, 2)`

```{r 'define-additive-model-structure'}
AdditiveStructure_EP_nested <- (
...
Speed_EP_AdditiveModel_nested <- lmer(
    AdditiveStructure_EP_nested,
    data = reading_speed_measure_data,
    control = lmerControl(optimizer = "bobyqa"),
    REML = TRUE
...
```{r}
summary(Speed_EP_AdditiveModel_nested)
# plot(Speed_EP_AdditiveModel_nested)
fixef(Speed_EP_AdditiveModel_nested)
```
...
The interactive model includes 14 parameters (including the intercept, main effects, interactions and random effects). Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r nrow(reading_speed_measure_data)`/14 = `r round(nrow(reading_speed_measure_data)/14, 2)`.

```{r 'define-interactive-structure'}
InteractiveStructure_EP_nested <- (
    "ReadingRate ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1.sc + Observation_lag2.sc + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag1.sc : Observation_lag2.sc + ReadingSessionNumber.sc : TimeInReadingSession.sc + ReadingSessionNumber.sc : PercentageLocation.sc + TimeInReadingSession.sc : PercentageLocation.sc + (1 | StoryId / UserId)"
)
```

```{r 'fit-interactive-model'}
Speed_EP_InteractiveModel_nested <- lmer(
    InteractiveStructure_EP_nested,
    data = reading_speed_measure_data,
    control = lmerControl(optimizer = "bobyqa"),
    REML = TRUE
...
```{r, include=FALSE}
summary(Speed_EP_InteractiveModel_nested)
# plot(Speed_EP_InteractiveModel_nested)
fixef(Speed_EP_InteractiveModel_nested)
```
...
```{r, include=FALSE}
check_collinearity(Speed_EP_InteractiveModel_nested)
plot(check_collinearity(Speed_EP_InteractiveModel_nested))
```
...
```{r, include=FALSE}
# cooksd <-
#     cooks.distance(Speed_EP_InteractiveModel_nested)
# plot(cooksd,
#     pch = "*",
#     cex = 2,
#     main = "Influential Obs by Cooks Distance"
...
```{r, include=FALSE}
check_outliers(Speed_EP_InteractiveModel_nested)
plot(check_outliers(Speed_EP_InteractiveModel_nested))
```
...
```{r, include=FALSE}
check_heteroscedasticity(Speed_EP_InteractiveModel_nested)
plot(check_heteroscedasticity(Speed_EP_InteractiveModel_nested))
```
...
```{r, include=FALSE}
random_intercept_UserId <- ranef(Speed_EP_InteractiveModel_nested)$UserId$`(Intercept)`
# qqnorm(random_intercept_UserId)
# qqline(random_intercept_UserId)
shapiro.test(random_intercept_UserId)
```
...
```{r, include=FALSE}
random_intercept_StoryId <- ranef(Speed_EP_InteractiveModel_nested)$StoryId$`(Intercept)`
# qqnorm(random_intercept_StoryId)
# qqline(random_intercept_StoryId)
shapiro.test(random_intercept_StoryId)
```
...
```{r, include=FALSE}
# plot(resid(Speed_EP_InteractiveModel_nested), fitted(Speed_EP_InteractiveModel_nested))
```
...
```{r, include=FALSE}
# qqnorm(residuals(Speed_EP_InteractiveModel_nested))
```

The qqplot shows slight variation from normality. However, the variation is not very extensive. Assess normality with a histogram of residuals:

```{r, include=FALSE}
# hist(resid(Speed_EP_InteractiveModel_nested), breaks = 100)
```
...
```{r}
# 0 - normal, neg - left skew, pos - right skew
skewness(resid(Speed_EP_InteractiveModel_nested))
# 3 - normal, <3 playkurtic, >3 leptokurtic
kurtosis(resid(Speed_EP_InteractiveModel_nested))
```
...
```{r, include=FALSE}
# Shapiro Wilk Test
check_normality(Speed_EP_InteractiveModel_nested)
```
...
speed_ep_previouslag1 <- effect_plot(
    Speed_EP_InteractiveModel_nested,
