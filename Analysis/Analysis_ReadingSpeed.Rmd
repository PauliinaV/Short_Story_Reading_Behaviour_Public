...
speed_effectplot_electronicexperiencescomp <-
    ...
    labs(
        y = "Reading Speed as Proportion of Baseline Speed",
        x = addline_format("Situational competence score,(centered and scaled)"),
        caption = addline_format("Legend variables:,Electronic experience 1:, Frequency of reading longform texts electronically,(centered and scaled), Elecronic experience 2:,Frequency of using task-relevant digital devices,for recreational reading (centered and scaled)")
    )
```
...
First, we select the fixed effects in the model backward stepwise until the model converges. We then add two-way interactions in the model, and select them backward stepwise until the model converges. Two-way interactions can be added between predictors measuring previous observations (Observation~k-1~ and Observation~k-2~) and between reading session number, time in reading session, and location in text. To avoid multicollinearity, interactions were not included between between previous observations and the other predictors in the model (e.g. Observation~k-1~ was not allowed to interact with reading session number, time in reading session, or location in text).
...
Only two-way interactions are included to avoid overfitting the model. No interactions are included between previous observations (Observation_lag1 and Observation_lag2) and other predictors in the model (e.g. ReadingSessionNumber) to avoid multicollinearity.
...
(1) Main effect of Observation_lag1
...
speed_ep_previouslag1 <- effect_plot(
    ...
    labs(
        y = "Reading Speed as Proportion of Baseline Speed",
        x = addline_format("Observation~k-1~:,Reading speed on previous page,(centered and scaled)")
    )
```

There is a slight positive trend which indicates that a high reading speed on previous page-view (observation k-1) predicts a high reading speed on the following page (observation k).

Interestingly, reading speed on the page before the previous (observation k-2) was not connected to reading speed (observation k).
...
speed_ep_previouslag2 <- effect_plot(
    ...
    labs(
        y = "Reading Speed as Proportion of Baseline Speed",
        x = addline_format("Observation~k-2~:,Reading speed on the page before previous page,(centered and scaled)")
    )
```
