...
First, we build the maximal interactive model structure.
...
Check the number of parameters against the number of observations in the model

```{r}
fepar <- lme4::getME(Speed_RC_nested_maximal, name = "p") # number of fixed effect parameters
repar <- lme4::getME(Speed_RC_nested_maximal, name = "m") # number of random effect parameters inc covariances
par <- fepar + repar
n <- lme4::getME(Speed_RC_nested_maximal, name = "n")
# top limits of analysis
## avoid overfitting with n/k => 15
n/par > 15
## limit computational requirements with 100 000 > 10*par^2
100000 > 10*par^2
```

The maximal model includes `r par` parameters, and `r n` observations. Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r n`/`r par` = `r round(n/par, 2)`
...
speed_effectplot_contextualcompetence
```
...
Check number of parameters against the number of observations in the additive, intercepts-only model:

```{r}
fepar_EP <- lme4::getME(Speed_EP_AdditiveModel_nested_interceptsonly, name = "p") # number of fixed effect parameters
repar_EP <- lme4::getME(Speed_EP_AdditiveModel_nested_interceptsonly, name = "m") # number of random effect parameters inc covariances
par_EP <- fepar_EP + repar_EP
n_EP <- lme4::getME(Speed_EP_AdditiveModel_nested_interceptsonly, name = "n")
# top limits of analysis
## avoid overfitting with n/k >= 15
n_EP/par_EP >= 15
## limit computational requirements with 100 000 > 10*par^2
...
The model includes `r par_EP` parameters before adding interactions, and `r n_EP` observations. Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r n_EP`/`r par_EP` = `r round(n_EP/par_EP, 2)`
...
```{r 'ep-backward-stepwise-selection-slopes', eval=FALSE}
...
Check amount of parameters against the number of observations:

```{r}
fepar_EP_slopes <- lme4::getME(AdditiveStructure_EP_nested_slope5_1, name = "p") # number of fixed effect parameters
repar_EP_slopes <- lme4::getME(AdditiveStructure_EP_nested_slope5_1, name = "m") # number of random effect parameters inc covariances
par_EP_slopes <- fepar_EP_slopes + repar_EP_slopes
n_EP_slopes <- lme4::getME(AdditiveStructure_EP_nested_slope5_1, name = "n")
# top limits of analysis
## avoid overfitting with n/k >= 15
n_EP_slopes/par_EP_slopes >= 15
## limit computational requirements with 100 000 > 10*par^2
100000 > 10*par_EP_slopes^2
```
...
Check number of parameters against the number of observations:

```{r}
fepar_EP_int <- lme4::getME(Speed_EP_InteractiveModel_nested_slopes, name = "p") # number of fixed effect parameters
repar_EP_int <- lme4::getME(Speed_EP_InteractiveModel_nested_slopes, name = "m") # number of random effect parameters inc covariances
par_EP_int <- fepar_EP_int + repar_EP_int
n_EP_int <- lme4::getME(Speed_EP_InteractiveModel_nested_slopes, name = "n")
# top limits of analysis
## avoid overfitting with n/k >= 15
n_EP_int/par_EP_int >= 15
## limit computational requirements with 100 000 > 10*par^2
100000 > 10*par_EP_int^2
```

The interactive model includes `r par_EP_int` parameters (including the intercept, random effects, main effects, interactions, and random effect covariances). Previous research has suggested that each parameter in a model has at least 15 observations. Our sample exceeds this threshold considering that `r n_EP_int`/`r par_EP_int` = `r round(n_EP_int/par_EP_int, 2)`.
