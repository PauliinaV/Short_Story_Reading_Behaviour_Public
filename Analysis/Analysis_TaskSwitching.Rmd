...
$Y_{i} = 
    ...
    TR-EExp2 +
    TR-EExp1 x TR-EExp2 +
    (1  + Cond + CMot + TR-EEXP1 x TR-EEXP2 | Story indicator) +
...
**(1  + Cond + CMot + TR-EEXP1 * TR-EEXP2 | Story indicator)** indicates a random effect of story indicator. The random effect includes an intercept for each story indicator, and a random slope for all key variables used to address our hypotheses.
...
```{r 'scale-continuous-variables', echo=FALSE}
# Situational competence
...
We build the maximal interactive model structure. The maximal model includes XX parameters, and `r nrow(task_switching_data)` observations. Previous research has suggested that each parameter in a model has 15 observations. Our sample is close to this threshold considering that `r nrow(task_switching_data)`/13 = `r round(nrow(task_switching_data)/13, 2)`

```{r 'define-maximal-structure'}
Structure_nested_maximal <- (
    "log(ContinuousEngagementMinutes + 1) ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc + (1 + Condition + CInterest.sc + Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc | StoryId) + (1 | UserId)"
)
```

```{r 'fit-maximal-model', echo=FALSE, warning="hide"}
TaskSwitching_nested_maximal <- lmer(
    Structure_nested_maximal,
...
The maximal model is singular, and so it is selected backward stepwise. We remove each random slope at a time until the model reaches convergence and is no longer singular.

```{r 'select-RC-model', echo=FALSE, warning='hide'}
# Identify random slope that accounts for the least variance in the model
summary(TaskSwitching_nested_maximal) # CInterest accounts for the least variance
# Remove random slope of CINT:
Structure_nested_selected1 <- (
    "log(ContinuousEngagementMinutes + 1) ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc + (1 + Condition + Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc | StoryId) + (1 | UserId)"
)
TaskSwitching_nested_selected1 <- lmer( # remains singular
    Structure_nested_selected1,
    data = task_switching_data,
    control = lmerControl(optimizer = "bobyqa"),
    REML = TRUE
)
# Identify the next random slope to be removed:
summary(TaskSwitching_nested_selected1) # TR-EEXP1 : TR-EEXP2 accounts for the least variance
# Remove random slope of TR-EEXP1 : TR-EEXP2
Structure_nested_selected2 <- (
    "log(ContinuousEngagementMinutes + 1) ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc + (1 + Condition | StoryId) + (1 | UserId)"
)
TaskSwitching_nested_selected2 <- lmer( # remains singular
    Structure_nested_selected2,
    data = task_switching_data,
    control = lmerControl(optimizer = "bobyqa"),
    REML = TRUE
)
# Remove the remaining random slope:
Structure_nested_selected3 <- (
    "log(ContinuousEngagementMinutes + 1) ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc + (1 | StoryId) + (1 | UserId)"
)
TaskSwitching_nested_selected3 <- lmer(
    Structure_nested_selected3,
    data = task_switching_data,
    control = lmerControl(optimizer = "bobyqa"),
    REML = TRUE
...
The model converges and is no longer singular after removal of all random slopes in the model.
Inspect model:

```{r, include=FALSE, echo=FALSE}
summary(TaskSwitching_nested_selected3)
plot(TaskSwitching_nested_selected3)
fixef(TaskSwitching_nested_selected3)
```
...
```{r, eval=FALSE, include=FALSE}
check_collinearity(TaskSwitching_nested_selected3)
# plot(check_collinearity(TaskSwitching_nested_selected3))
```
...
```{r, eval=FALSE, include=FALSE}
check_heteroscedasticity(TaskSwitching_nested_selected3)
# plot(check_heteroscedasticity(TaskSwitching_nested_selected3))
```
...
```{r, eval=FALSE}
leveneTest(residuals(TaskSwitching_nested_selected3) ~ task_switching_data$Condition)
```

```{r, eval=FALSE}
leveneTest(residuals(TaskSwitching_nested_selected3) ~ task_switching_data$NativeEnglish)
```
...
```{r, eval=FALSE, include=FALSE}
# cooksd <-
#      cooks.distance(TaskSwitching_nested_selected3)
# plot(cooksd,
#    pch = "*",
#     cex = 2,
...
```{r, eval=FALSE, include=FALSE}
check_outliers(TaskSwitching_nested_selected3)
plot(check_outliers(TaskSwitching_nested_selected3))
```
...
```{r, eval=FALSE, include=FALSE}
random_intercept_UserId <- ranef(TaskSwitching_nested_selected3)$UserId$`(Intercept)`
...
```{r, eval=FALSE, include=FALSE}
random_intercept_StoryId <- ranef(TaskSwitching_nested_selected3)$StoryId$`(Intercept)`
...
We check normality of residuals by plotting model residuals against fitted values

```{r}
plot(resid(TaskSwitching_nested_selected3), fitted(TaskSwitching_nested_selected3))
```

The model residuals seem to be largely symmetrically scattered in the plot.

To further check non-normality, we use a qqplot:

```{r}
qqnorm(residuals(TaskSwitching_nested_selected3))
```

The qqplot shows slight variation from normality. Inspect it using a histogram:

```{r}
hist(resid(TaskSwitching_nested_selected3), breaks = 100)
```

The histogram shows some variation from a normal distribution. We then inspect skewness and kurtosis in residuals:

```{r}
# 0 - normal, neg - left skew, pos - right skew
skewness(resid(TaskSwitching_nested_selected3)) # slight left skew
# 3 - normal, <3 playkurtic, >3 leptokurtic
kurtosis(resid(TaskSwitching_nested_selected3)) # playkurtic
```

Finally, we test normality with a Shapiro Wilkins test:

```{r}
check_normality(TaskSwitching_nested_selected3)
```

Shapiro Wilk test is not significant, and so we can assume normality of residuals.
...
taskswitching_effectplot_condition <- effect_plot(
    TaskSwitching_nested_selected3,
    pred = "Condition",
    data =  task_switching_data,
    plot.points = TRUE,
    point.size = 2,
