...
* H5a: Situational autonomous motivation is connected to frequent nonlinearity when task competence is low, and less frequent nonlinearity when task competence is high.
* H5b: Contextual autonomous motivation is connected to frequent nonlinearity when task competence is low, and less frequent nonlinearity when task competence is high.
* H5c: Task-relevant electronic experience is connected to frequent nonlinearity when task competence is low, and less frequent nonlinearity when task competence is high.
...
```{r 'load helmert contrasts functions rc'}
source(
    paste0(
        mypath_SSRBP,
...
First, we build the full structure. Refer to the beginning of this script to see information on the full structure. The full model includes 16 parameters, and `r nrow(linearity_measure_data)` observations. Previous research has suggested that each parameter in a model has at least 20 observations. Our sample exceeds this threshold considering that `r nrow(linearity_measure_data)`/16 = `r round(nrow(linearity_measure_data)/16, 2)`
...
Inspection of the model values indicates that the random intercept of StoryId is accounting for zero variance in the model. This can occur when the variance accounted for by the random intercept is minimal and thus the model fails to estimate variance, resulting in singularity. To address the issue, we attempt to fit the model with StoryId as a fixed effect rather than a random intercept:

```{r}
FullStructure_RC_AdjustedStoryId <- (
...
```{r}
FullStructure_RC_RemovedStoryId <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + NativeEnglish + Condition + SCompetence.sc + CInterest.sc + CCompetence.sc + Eexp1LongformEFreq.sc + Eexp2DeviceRecFreq.sc +  Condition : SCompetence.sc + CInterest.sc : SCompetence.sc + Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc +  Eexp1LongformEFreq.sc : Eexp2DeviceRecFreq.sc : SCompetence.sc + (1 | UserId)"
)
```
...
```{r}
Linearity_RC_Model_RemovedStoryId <- glmer(
    FullStructure_RC_RemovedStoryId,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
...
```{r}
car::vif(Linearity_RC_Model_RemovedStoryId)
```
...
```{r}
check_collinearity(Linearity_RC_Model_RemovedStoryId)
plot(check_collinearity(Linearity_RC_Model_RemovedStoryId))
```
...
```{r}
simulated_model_output <- simulateResiduals(fittedModel = Linearity_RC_Model_RemovedStoryId, plot = F)
```
...
```{r}
cooksd <-
    cooks.distance(Linearity_RC_Model_RemovedStoryId)
plot(cooksd,
    pch = "*",
    cex = 2,
...
```{r}
random_intercept_UserId <- ranef(Linearity_RC_Model_RemovedStoryId)$UserId$`(Intercept)`
...
```{r}
summary(Linearity_RC_Model_RemovedStoryId)
```
...
linearity_rc_condscomp <- interact_plot(
    Linearity_RC_Model_RemovedStoryId,
    pred = SCompetence.sc,
    interval = TRUE,
    modx = Condition,
    data = linearity_measure_data
...
As the interaction effect was not significant, the combination of situational competence and condition manipulation was not connected to participants' frequency of using nonlinear navigation. Therefore, our hypothesis H5a was not supported.
...
linearity_rc_cmotscomp <- interact_plot(
    ...
    data =  linearity_measure_data,
    interval = TRUE,
    legend.main = addline_format("Contextual motivation score,(centered and scaled)")
) +
    theme_bw() +
    theme(
        panel.grid.major = element_blank(),
...
```{r 'load helmert contrasts functions ep'}
source(
    paste0(
        mypath_SSRBP,
        "/Functions/Functions_contr.helmert.weighted.R"
...
First, we build the additive model structure consisting of only main effects and random intercepts. Refer to the beginning of the event properties section of this script to see information on how the model is built.
...
AdditiveStructure_EP <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + StartsNonlinearity_lag1 + StartsNonlinearity_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 | StoryId) + (1 | UserId)"
)
```
...
The additive model is singular, and so we inspect the model values:
...
Similarly to the reader characteristics model, story indicator accounts for zero variance in the event properties model of linearity. This is likely to be causing the singularity in the model. To address the issue, we attempt to fit StoryId as a fixed effect in the model:

```{r}
AdditiveStructure_EP_Adjusted <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + StartsNonlinearity_lag1 + StartsNonlinearity_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 | UserId)"
)
```
...
Re-fit the model with StoryId as a fixed effect:
...
The adjusted model reaches convergence and is no longer singular.
Before proceeding, we assess if StoryId is causing multicollinearity in the model:

```{r}
car::vif(Linearity_EP_AdditiveModel_Adjusted)
```

StoryId VIF is within reasonable limits, and therefore it can be retained in the model as a fixed effect.

Considering that the model converges, we can add two-way interactions.

#### Interactions

We create a new model structure with interactions between all predictors of interest (see information at the beginning of the event properties section for an explanation of model selection). Only two-way interactions are included to avoid overfitting the model. Note that StoryId is fit as a fixed rather than a random effect due to singularity in the additive model structure. The interactive model includes 21 parameters. Previous research has suggested that each parameter in a model has at least 20 observations. Our sample exceeds this threshold considering that `r nrow(linearity_measure_data)`/21 = `r round(nrow(linearity_measure_data)/21, 2)`.

```{r}
InteractiveStructure_EP_Adjusted <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + StartsNonlinearity_lag1 + StartsNonlinearity_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + StartsNonlinearity_lag1 : StartsNonlinearity_lag2 + ReadingSessionNumber.sc : TimeInReadingSession.sc + ReadingSessionNumber.sc : PercentageLocation.sc + TimeInReadingSession.sc : PercentageLocation.sc + (1 | UserId)"
)
```
...
simulated_model_output <- simulateResiduals(fittedModel = Linearity_EP_InteractiveModel_Adjusted, plot = F)
```
...
The plot indicates that the dataset may include some highly influential observations. In particular, the plot highlights observation #254.

We further inspect outliers using the simulated_model_output and DHARMa's outlier test:

```{r}
testOutliers(Linearity_EP_InteractiveModel_Adjusted)
```

Although Cook's Distance indicates that the model has influential observations, the outlier test does not produce the same results. To assess the impact of observation #254, we attempt to fit the model without it to see if its inclusion has an impact on model interpreations:
...
```{r}
Linearity_EP_InteractiveModel_Adjusted_influential254 <- glmer(
    InteractiveStructure_EP_Adjusted,
...
```{r}
summary(Linearity_EP_InteractiveModel_Adjusted_influential254)
summary(Linearity_EP_InteractiveModel_Adjusted)
```

Removing observation #254 has an impact on model results. Removing the value removes a significant main effect (PercentageLocation). Removing the observation also decreases the significance of TimeInReadingSession and an interaction between TimeInReadingSession and PercentageLocation.

Therefore, the observation affects model interpretation and it should be removed from analyses.

Assess influential observations after removing point 254:

```{r}
cooksd <-
    cooks.distance(Linearity_EP_InteractiveModel_Adjusted_influential254)
plot(cooksd,
    pch = "*",
    cex = 2,
    main = "Influential Obs by Cooks Distance"
...
The plot indicates that the dataset still includes some influential observations. However, the influential observations' Cook's Distance has considerably reduced. Experimentation off-script indicated that the most prominently highlighted observations in the plot (#283 and #60) did not have an influence on model interpretation and so they can be retained in the model.
...
```{r}
set.seed(1234) # for reproducibility
simulated_model_output <- simulateResiduals(fittedModel = Linearity_EP_InteractiveModel_Adjusted_influential254, plot = F)
```
...
```{r}
random_intercept_UserId <- ranef(Linearity_EP_InteractiveModel_Adjusted_influential254)$UserId$`(Intercept)`
qqnorm(random_intercept_UserId)
qqline(random_intercept_UserId)
shapiro.test(random_intercept_UserId)
```
...
One influential observation was found in assumption testing. The observations was removed from the model dataset and the model was re-fit.

The new model is re-tested for the assumptions of multicollinearity and dispersion that were tested before the influential observation was removed:

1. Multicollinearity:

```{r}
check_collinearity(Linearity_EP_InteractiveModel_Adjusted_influential254)
plot(check_collinearity(Linearity_EP_InteractiveModel_Adjusted_influential254))
```

The tests indicate that the adjusted model without the influential observation does not have significant multicollinearity.

2. Over- and under-dispersion

```{r}
# simulated_model_output was created above for the adjusted model without the influential point
plotQQunif(simulated_model_output)
```
...
After the removal of one influential observation the model aligns will assumptions.
...
2. A main effect of previous event (navigation event k-1, 'StartsNonlinearity_lag1')
3. A main effect of the event preceeding previous (navigation event k-2, 'StartsNonlinearity_lag2')
4. A main effect of time since the beginning of a reading session ('TimeInReadingSession')
5. An interaction effect between reading session number and location in text
6. An interaction effect between time in a reading session and location in text

These findings are explored below by visualisations.
First, we discuss the effect of screen size (1). Second, we discuss the effect of previous events (2 and 3). Third, we discuss the influence of reading session and location (4, 5, and 6)
...
linearity_ep_windowwidth <- effect_plot(
    Linearity_EP_InteractiveModel_Adjusted_influential254,
    pred = "WindowWidth.sc",
    data =  subset_without_observation254,
    interval = TRUE
) +
...
Window width and nonlinearity are positively connecting, indicating that the odds of initiating nonlinearity increase when the e-reader is used on a larger device.
...
linearity_ep_previousevent <- effect_plot(
    ...
    labs(
        y = addline_format("Odds of initiating nonlinear navigation,(Navigation Event~k~)"),
        x = addline_format("Whether previous event initiated,nonlinear navigation or not,(Navigation Event~k-1~)")
    )
```
...
In addition to the event at event~k-1~, the event~k-2~ was a significant predictor of nonlinearity:

```{r}
linearity_ep_lag2 <- effect_plot(
    Linearity_EP_InteractiveModel_Adjusted_influential254,
    pred = "StartsNonlinearity_lag2",
    interval = TRUE,
    ...
    labs(
        y = addline_format("Odds of initiating nonlinear navigation,(Navigation Event~k~)"),
        x = addline_format("Whether nonlinear navigation was initiated,in the event preceeding the previous or not,(Navigation Event~k-2~)")
    )
```

The odds of a participant initiating nonlinearity at navigation event~k~ were higher if the event that preceeded its previous (event~k-2~) also initiated nonlinearity. This is in contrast to the main effect between linearity and navigation event~k-1~ which indicated that the odds of initiating nonlinearity were higher if the previous event (event~k-1~) did not initiate nonlinearity. The finding indicates that although participants are unlikely to vary their linearity of reading in consecutive pages (event~k~ and event~k-1~), changes in linearity are common (e.g. between event~k~ and event~k-2~).

#### Effect of Reading Sessions, Their Timing, and Location in Text
...
Finally, linearity of reading was predicted by an interaction effect between reading session number and location in text.

```{r}
linearity_ep_locationRSNumb <- interact_plot(
    Linearity_EP_InteractiveModel_Adjusted_influential254,
    pred = "PercentageLocation.sc",
    modx = "ReadingSessionNumber.sc",
    legend.main = addline_format("Reading session number,(centered and scaled)"),
    interval = TRUE
) +
    theme_bw() +
    theme(
...
The interaction shows that nonlinear navigation was most likely in early reading sessions if the participants was reading the beginning of the text. However, the odds of initiating nonlinear navigation decrease further in the text in early reading sessions. In contrast, the odds of nonlinear navigation stay constant across the location of the text in latter reading sessions.
