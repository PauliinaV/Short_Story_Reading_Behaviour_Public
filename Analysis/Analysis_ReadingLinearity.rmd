...
First, we select the fixed effects in the model backward stepwise until the model converges. We then add two-way interactions in the model, and select them backward stepwise until the model converges. Two-way interactions could be added between predictors measuring previous observations (Observation~k-1~ and Observation~k-2~) and between reading session number, time in reading session, and location in text. To avoid multicollinearity, interactions were not included between between previous observations and the other predictors in the model (e.g. Observation~k-1~ was not allowed to interact with reading session number, time in reading session, or location in text).
...
2. A main effect of previous event (observation k-1, 'StartsNonlinearity_lag1')
3. A main effect of the event preceeding previous (observation k-2, 'StartsNonlinearity_lag2')
...
linearity_ep_previousevent <- effect_plot(
    ...
    labs(
        y = addline_format("Odds of initiating nonlinear navigation,(Observation~k~)"),
        x = addline_format("Whether previous event initiated,nonlinear navigation or not,(Observation~k-1~)")
    )
```
...
In addition to the event at observation~k-1~, the observation~k-2~ was a significant predictor of nonlinearity:
...
linearity_ep_lag2 <- effect_plot(
    ...
    labs(
        y = addline_format("Odds of initiating nonlinear navigation,(Observation~k~)"),
        x = addline_format("Whether nonlinear navigation was initiated,in the event preceeding the previous or not,(Observation~k-2~)")
    )
```

The odds of a participant initiating nonlinearity at observation~k~ were higher if the observation that preceeded its previous (observation~k-2~) also initiated nonlinearity. This is in contrast to the main effect between linearity and observation~k-1~ which indicated that the odds of initiating nonlinearity were higher if the previous event (observation~k-1~) did not initiate nonlinearity. The finding indicates that although participants are unlikely to vary their linearity of reading in consecutive pages (observation~k~ and observation~k-1~), changes in linearity are common (e.g. between observation~k~ and observation~k-2~).
