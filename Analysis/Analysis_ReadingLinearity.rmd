...
```{r 'create-preceeding-event-variables', eval=FALSE}
...
anova(Linearity_EP_AdditiveModel_nested1_7, Linearity_EP_AdditiveModel) # contributes, p = .292
```
...
# Model without slope for readomg session number
...
Linearity_EP_AdditiveModel_nested6b_2 <- glmer( # sing, nc
    AdditiveStructure_EP_nested_slope5b_2,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
anova(Linearity_EP_AdditiveModel_nested6, Linearity_EP_AdditiveModel_nested6b_2) # does not contribute
```

None of the remaining variables contribute to the model as random effects.
Inspect the adjusted additive model with random slopes: CONTINUE HERE
...
The model is nonconvergent and singular, and so we attempt to resolve it by simplifying the structure backward-stepwise, starting with the random slopes. PercentageLocation accounts for the least variance in the model, and so it is removed first.
