...
```{r}
AdditiveStructure_EventProperties <- (
...
```{r}
InteractiveStructure_EventProperties <- (
...
```{r}
corrplot(cor(linearity_measure_data[, c(40:42)]), method = "number")
```
...
Despite of the correlations, and the multiple interactions included in the model, the predictors do not seem to be multicollinear. All VIF < 2.5, and usually VIF > 5 is used as an indicator of potential multicollinearity.
...
The p-value is not significant, although it is close to .05 at .08. The plot shows that some quantile deviations may be present as the lowest reference line in the plot does not lie parallel to the reference lines. However, this variance is minimal and so it is judged to not require corrections.
...
The plot indicates that the datset includes some highly influential observations. The plot highlights three influential values, numbers 60, 254, and 263.
...
Although Cook's Distance indicates that the model has influential observations, the outlier test does not produce the same results. In order to assess the impact of the influential points indicated by Cook's Distance, we re-fit the model three times without each influential observation to see if the results are altered.

**Fit the model without observation #263:**

```{r}
subset_without_observation263 <- linearity_measure_data[c(1:262, 264:nrow(linearity_measure_data)), ]
```

```{r}
Linearity_EP_InteractiveModel_influential263 <- glmer(
    InteractiveStructure_EventProperties,
    data = subset_without_observation263,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
```
...
```{r}
summary(Linearity_EP_InteractiveModel_influential263)
summary(Linearity_EP_InteractiveModel)
```

Removing observation #263 does not seem to impact results as the significance of effects are not altered. Inclusion of this observation does not affect model interpretation.
...
Removing observation #254 has an impact on model results. Removing the value results in a new significant interaction effect (StartsNonlinearity_lag2 and TimeInReadingSession.s). Therefore, the observation affects model interpretation and it should be removed from analyses.

**Fit the model without observation #60:**

```{r}
subset_without_observation60 <- linearity_measure_data[c(1:59, 61:nrow(linearity_measure_data)), ]
```

```{r}
Linearity_EP_InteractiveModel_influential60 <- glmer(
    InteractiveStructure_EventProperties,
    data = subset_without_observation60,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
```
...
```{r}
summary(Linearity_EP_InteractiveModel_influential60)
summary(Linearity_EP_InteractiveModel)
```

Removing observation #60 does not seem to impact results as the significance of effects are not altered. Inclusion of this observation does not affect model interpretation.

In conclusion, observation #254 is treated as an influential point. It is removed from analysis at the end of the assumptions.
...
One influential observation was found in assumption testing. This observation was removed from the model dataset and the model was re-fit. In interpretations we use Linearity_EP_InteractiveModel_influential254 which does not contain the influential observation. The new model was re-checked for assumptions (see plots below). The same tests were conducted, and the model was found the align with assumptions. Indeed, without the influential observation, the model aligned better with the assumptions, for example, in the case of heteroscedasticity.

```{r 'all assumptions with updated model'}
plot(check_collinearity(Linearity_EP_InteractiveModel_influential254))
simulated_model_output <- simulateResiduals(fittedModel = Linearity_EP_InteractiveModel_influential254, plot = F)
plotQQunif(simulated_model_output)
testDispersion(simulated_model_output)
testQuantiles(simulated_model_output)
testOutliers(simulated_model_output)
random_intercept_UserId <- ranef(Linearity_EP_InteractiveModel_influential254)$UserId$`(Intercept)`
shapiro.test(random_intercept_UserId)
random_intercept_StoryId <- ranef(Linearity_EP_InteractiveModel_influential254)$StoryId$`(Intercept)`
shapiro.test(random_intercept_StoryId)
random_intercept_WindowWidth <- ranef(Linearity_EP_InteractiveModel_influential254)$WindowWidth$`(Intercept)`
shapiro.test(random_intercept_WindowWidth)
```
