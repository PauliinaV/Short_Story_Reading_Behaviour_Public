...
$Y_{i} = 
    ...
    (1 + WindowWidth + DUD | Story indicator / Participant indicator) +
    \varepsilon_{i}$ 
...
**(1 + WindowWidth + DUD | Story indicator / Participant indicator)** indicates a nested random intercept of story and subject indicator with random slopes for Window width and DUD. Note that only these random slopes are included as other fixed effects in the model only have one observation for each participant. The random effects are nested because each participant only read one short story during the study. The nesting is done explicitly, and therefore, each participant indicator is unique, which makes the model identical regardless if a nested (e.g., (1|Story/Participant)) or crossed (e.g., (1|Story) + (1|Participant)) notation is used. We prefer the notation of (1|Story/Participant) to make it clear that the random effects are nested, however, in backward selection of the slopes, the model may occasionally be specified with the crossed notation instead.
...
```{r 'scale-continuous-variables-rc'}
# Situational competence
...
```{r 'backward-select-slopes-in-model'}
...
First, we select the fixed effects in the model backward stepwise until the model converges. We then add two-way interactions in the model, and select them backward stepwise until the model converges. Two-way interactions could be added between predictors measuring previous navigation events (Event~k-1~ and Event~k-2~) and between reading session number, time in reading session, and location in text. To avoid multicollinearity, interactions were not included between between previous navigation events and the other predictors in the model (e.g. Event~k-1~ was not allowed to interact with reading session number, time in reading session, or location in text).
...
```{r, include=FALSE}
table(linearity_measure_data$StartsNonlinearity, linearity_measure_data$Observation_lag1)
ggplot(
    linearity_measure_data,
...
```{r 'scale-continuous-variables-ep'}
...
```{r 'fit-the-intercept-only-additive-model'}
...
anova(Linearity_EP_AdditiveModel, Linearity_EP_AdditiveModel_nested1_1) # does not contribute, p > .2
# 1_2
...
anova(Linearity_EP_AdditiveModel_nested1_3, Linearity_EP_AdditiveModel) # contributes, p = .101
# 1_4
...
anova(Linearity_EP_AdditiveModel_nested1_4, Linearity_EP_AdditiveModel) # contributes, p = .178
# 1_5
...
anova(Linearity_EP_AdditiveModel_nested1_5, Linearity_EP_AdditiveModel) # contributes, p = .169
# 1_6
...
anova(Linearity_EP_AdditiveModel_nested1_7, Linearity_EP_AdditiveModel) # contributes, p = .145
```
...
Linearity_EP_AdditiveModel_nested2_1 <- glmer( # sing, nc
    AdditiveStructure_EP_nested_slope2_1,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2_1) # does not contribute, p > .2
# 2_2
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2_5) # contributes, p = .172
# 2_6
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2_6) # contributes, p = .142
```

A random slope of PercentageLocation contributes most to the model. We continue testing possible random slopes by comparing to a model with a random slope for TimeInReadingSession and PercentageLocation

```{r 'select-slopes2'}
# slope test 3
## Start adding all slopes to a model with TRS + LOC
    ## 3_1: WW
    ## 3_2: DUD
    ## 3_3: Event k-1
    ## 3_4: Event k-2
...
AdditiveStructure_EP_nested_slope3_1 <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + WindowWidth.sc | StoryId / UserId)"
)
...
AdditiveStructure_EP_nested_slope3_2 <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + DaysUntilDeadline.sc | StoryId / UserId)"
)
...
AdditiveStructure_EP_nested_slope3_3 <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag1 | StoryId / UserId)"
)
...
AdditiveStructure_EP_nested_slope3_4 <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 | StoryId / UserId)"
)
...
AdditiveStructure_EP_nested_slope3_5 <- (
    "StartsNonlinearity ~ WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + ReadingSessionNumber.sc | StoryId / UserId)"
)
...
None of the remaining random slopes contributes to the model, and so the additive model has slopes for time in a reading session and percentage location only.

Inspect the model:

```{r 'additive-model-with-slopes'}
...
Similarly to the reader chatacteristics model, we first attempt to make the model non-singular by removing random slopes that least contribute to the model.

Inspection of the model shows that TimeInReadingSession contributes less to the model, and so it is removed first.

```{r 'backward-select-slopes'}
...
# Model without slope for location
...
anova(Linearity_EP_AdditiveModel_Adjusted, Linearity_EP_AdditiveModel_nested1b_1) # contributes, p = .139
# 1b_2
...
anova(Linearity_EP_AdditiveModel_Adjusted, Linearity_EP_AdditiveModel_nested1b_3) # contributes, p = .0071
# 1b_4
...
anova(Linearity_EP_AdditiveModel_Adjusted, Linearity_EP_AdditiveModel_nested1b_4) # contributes, p = .072
# 1b_5
...
anova(Linearity_EP_AdditiveModel_Adjusted, Linearity_EP_AdditiveModel_nested1b_5) # contributes, p = .112
# 1b_6
...
anova(Linearity_EP_AdditiveModel_Adjusted, Linearity_EP_AdditiveModel_nested1b_7) # contributes, p = .034
```
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2b_1) # contributes, p = .033
# 2b_2
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2b_3) # contributes, p = .083
# 2b_4
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2b_4) # contributes, p = .194
# 2b_5
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2b_5) # contributes, p = .065
# 2b_6
...
anova(Linearity_EP_AdditiveModel_nested2, Linearity_EP_AdditiveModel_nested2b_6) # contributes, p = .027
```

PercentageLocation contributes the most, and so it is added as a random slope in the model. Next, we test the remaining variables as slopes against a model with TRS and LOC

```{r 'select-slopes-adjusted3'}
# adjusted model slope test 3
## Compare to model with a random slope of TRS and LOC
    ## 3b_1 WW
    ## 3b_2 DUD
    ## 3b_3 Event k-1
    ## 3b_4 Event k-2
...
Linearity_EP_AdditiveModel_nested3 <- glmer(
    AdditiveStructure_EP_nested_slope3,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
...
AdditiveStructure_EP_nested_slope3b_1 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + WindowWidth.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested3, Linearity_EP_AdditiveModel_nested3b_1) # contributes, p = .077
# 3b_2
AdditiveStructure_EP_nested_slope3b_2 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + DaysUntilDeadline.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested3, Linearity_EP_AdditiveModel_nested3b_2) # does not contribute, p > .2
# 3b_3
AdditiveStructure_EP_nested_slope3b_3 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag1 | UserId)"
)
Linearity_EP_AdditiveModel_nested3b_3 <- glmer( # sing
    AdditiveStructure_EP_nested_slope3b_3,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
...
AdditiveStructure_EP_nested_slope3b_4 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 | UserId)"
)
Linearity_EP_AdditiveModel_nested3b_4 <- glmer(
    AdditiveStructure_EP_nested_slope3b_4, # sing
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
anova(Linearity_EP_AdditiveModel_nested3, Linearity_EP_AdditiveModel_nested3b_4) # contributes, p = .051
# 3b_5
AdditiveStructure_EP_nested_slope3b_5 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + ReadingSessionNumber.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested3, Linearity_EP_AdditiveModel_nested3b_5) # contributes, p = .125
```

Observation_lag2 contributes the most, and so it is added as a random slope in the model. Next, we test the remaining variables as slopes against a model with TRS, LOC and Obs lag2

```{r 'select-slopes-adjusted4'}
# adjusted model slope test 4
## Compare to model with a random slope of TRS, LOC and OBS lag2
    ## 4b_1 WW
    ## 4b_2 DUD
    ## 4b_3 Event k-1
    ## 4b_4 RSN
...
AdditiveStructure_EP_nested_slope4b_1 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + WindowWidth.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested4, Linearity_EP_AdditiveModel_nested4b_1) # contributes, p = .073
# 4b_2
AdditiveStructure_EP_nested_slope4b_2 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + DaysUntilDeadline.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested4, Linearity_EP_AdditiveModel_nested4b_2) # does not contribute, p > .2
# 4b_3
AdditiveStructure_EP_nested_slope4b_3 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + Observation_lag1 | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested4, Linearity_EP_AdditiveModel_nested4b_3) # contributes, p = .099
# 4b_4
AdditiveStructure_EP_nested_slope4b_4 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + ReadingSessionNumber.sc | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested4, Linearity_EP_AdditiveModel_nested4b_4) # does not contribute, p > .2
```

WindowWidth contributes the most, and so it is added as a random slope in the model. Next, we test the remaining variables as slopes against a model with TRS, LOC, Obs lag2, and WW

```{r 'select-slopes-adjusted5'}
# adjusted model slope test 5
## Compare to model with a random slope of TRS, LOC, OBS lag2, and WW
    ## 5b_1 DUD
    ## 5b_2 Event k-1
    ## 5b_3 RSN
###################
...
AdditiveStructure_EP_nested_slope5b_1 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + WindowWidth.sc + DaysUntilDeadline.sc | UserId)"
)
...
AdditiveStructure_EP_nested_slope5b_2 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + WindowWidth.sc + Observation_lag1 | UserId)"
)
...
anova(Linearity_EP_AdditiveModel_nested5, Linearity_EP_AdditiveModel_nested5b_2) # does not contribute, p > .2
# 5b_3
AdditiveStructure_EP_nested_slope5b_3 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag2 + WindowWidth.sc + ReadingSessionNumber.sc | UserId)"
)
...
Inspect the adjusted additive model with random slopes:

```{r}
summary(Linearity_EP_AdditiveModel_nested5)
```

The model is singular, and so we attempt to resolve it by simplifying the structure backward stepwise, starting with the random slopes. PercentageLocation accounts for the least variance in the model, and so it is removed first.

```{r 'adjusted-model-backward-select-slopes'}
# fit the model without PercentageLocation
AdditiveStructure_EP_Adjusted_slope_selected1 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + Observation_lag2 + WindowWidth.sc | UserId)"
)
Linearity_EP_AdditiveModel_Adjusted_slope_selected1 <- glmer( # remains singular
    AdditiveStructure_EP_Adjusted_slope_selected1,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
summary(Linearity_EP_AdditiveModel_Adjusted_slope_selected1) # WindowWidth accounts for the least variance
# fit the model without WindowWidth
AdditiveStructure_EP_Adjusted_slope_selected2 <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + (1 + TimeInReadingSession.sc + Observation_lag2 | UserId)"
)
Linearity_EP_AdditiveModel_Adjusted_slope_selected2 <- glmer( # no longer singular
    AdditiveStructure_EP_Adjusted_slope_selected2,
    data = linearity_measure_data,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
...
After removal of two random slopes, PercentageLocation and WindowWidth, the model converges and is no longer singular.

Inspect the model:

```{r}
summary(Linearity_EP_AdditiveModel_Adjusted_slope_selected2)
plot(Linearity_EP_AdditiveModel_Adjusted_slope_selected2)
fixef(Linearity_EP_AdditiveModel_Adjusted_slope_selected2)
```
...
InteractiveStructure_EP_Adjusted <- (
    "StartsNonlinearity ~ StoryId + WindowWidth.sc + DaysUntilDeadline.sc + Observation_lag1 + Observation_lag2 + ReadingSessionNumber.sc + TimeInReadingSession.sc + PercentageLocation.sc + Observation_lag1 : Observation_lag2 + ReadingSessionNumber.sc : TimeInReadingSession.sc + ReadingSessionNumber.sc : PercentageLocation.sc + TimeInReadingSession.sc : PercentageLocation.sc + (1 + TimeInReadingSession.sc + Observation_lag2 | UserId)"
)
```
...
plot(Linearity_EP_InteractiveModel_Adjusted)
fixef(Linearity_EP_InteractiveModel_Adjusted)
```
...
The model predictors do not seem to be multicollinear. All VIF < 4, and usually VIF > 5 is used as an indicator of potential multicollinearity.
...
The plot indicates that the dataset may include some highly influential observations. In particular, the plot highlights observation #3462.
...
Although Cook's Distance indicates that the model has influential observations, the outlier test does not produce the same results. To assess the impact of observation #3462, we attempt to fit the model without it to see if its inclusion has an impact on model interpreations:

**Fit the model without observation #3462:**

```{r}
subset_without_observation3462 <- linearity_measure_data[c(1:3461, 3463:nrow(linearity_measure_data)), ]
```

```{r 'fit-model-wo-obs-3462'}
Linearity_EP_InteractiveModel_Adjusted_influential3462 <- glmer(
    InteractiveStructure_EP_Adjusted,
    data = subset_without_observation3462,
    family = binomial(),
    control = glmerControl(optimizer = "bobyqa")
)
```
...
```{r}
summary(Linearity_EP_InteractiveModel_Adjusted_influential3462)
summary(Linearity_EP_InteractiveModel_Adjusted)
```

The results are not influenced by removing observation #3462, and so we retain it in the sample. Considering that the most influential observation does not appear to affect model results, we believe that the sample does not have any influential observations.
...
The p-value is not significant, and the lines lie flat and horizontal in the plot, close to the reference lines. This indicates that the assumption of heteroscedasticity is fulfilled.
...
The model was fit with StoryId as a fixed rather than a random effect to resolve singularity in the model that was due to zero variance accounted for by the StoryId random intercept.
The adjusted model aligns will assumptions.
...
1. A main effect of screensize (WindowWidth)
2. A main effect of previous event (navigation event k-1, 'Observation_lag1')
3. A main effect of time since the beginning of a reading session ('TimeInReadingSession')
4. An interaction effect between time in a reading session and location in text

These findings are explored below by visualisations.
First, we discuss the effect of screen size (1). Second, we discuss the effect of previous events (2). Third, we discuss the influence of reading session and location (3 and 4)
...
WindowWidth represents the width of a browser screen, and thus it is used as an indication of the participants' device (e.g. smartphones have smaller window widths than laptops with the browser window maximised to fit the screen), and text amount (larger windows have more text). WindowWidth was used only as a control variable in the model to account for the impact of device size and text layout, however, the main effect was a significant predictor of nonlinear navigation.

```{r, warning='hide'}
linearity_ep_windowwidth <- effect_plot(
    ...
    labs(
        y = "Odds of initiating nonlinear navigation",
        x = addline_format("Window width in pixels,(centered and scaled)")
    )
```

Window width and nonlinearity are positively connected, indicating that the odds of initiating nonlinearity increase when the e-reader is used on a larger device.
...
The interaction effect indicates that the odds of nonlinear navigation are the highest if the participant is at the beginning of the story at the beginning of a reading session. However, the odds of nonlinear navigation decrease if the participant is reading the end of a text at the beginning of a reading session. In contrast, the odds of nonlinear navigation stay constant across time since the beginning of a reading session if a participant is viewing the end of the story.