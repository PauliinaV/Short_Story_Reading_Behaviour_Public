...
## 4. Progress graphs for one story with 5 random participants
...
Randomly select five participants who read StoryId 1:

```{r}
set.seed(122)
five_participants <- sample(unique(StoryId1$UserId), 5)
```

Re-create a figure for StoryId 1 with only these five participants:

```{r}
StoryId1_sample <- filter(StoryId1,
    UserId == five_participants[1] |
    UserId == five_participants[2] |
    UserId == five_participants[3] |
    UserId == five_participants[4] |
    UserId == five_participants[5])
```
...
```{r, eval=FALSE}
ggsave(
    "Fig_ProgressGraphStoryId1b.png",
    plot = Graph4e,
    width = 5,
    height = 5,
    units = "in",
